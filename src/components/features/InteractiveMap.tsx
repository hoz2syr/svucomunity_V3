...
export const InteractiveMapSimulation = () => {
  ...
  useEffect(() => {
    ...
    switch (step) {
      case 0: timer = setTimeout(nextStep, 500); break;
      case 1: timer = setTimeout(nextStep, 500); break;
      case 2: timer = setTimeout(nextStep, 600); break;
      case 3: timer = setTimeout(nextStep, 300); break;
      case 4: timer = setTimeout(nextStep, 800); break;
  ...
  return (
    <div className="w-full h-full bg-transparent overflow-hidden relative select-none">
        ...
        <div className={`absolute inset-0 z-10 transition-transform duration-[1200ms] ease-[cubic-bezier(0.22,1,0.36,1)] origin-center ${isZoomed ? 'scale-[1.15] md:-translate-x-[10%] -translate-y-[10%]' : 'scale-100 translate-x-0 translate-y-0'}`}>
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none drop-shadow-xl" viewBox="0 0 400 300" preserveAspectRatio="none">
            ...
            <path d="M 200 150 H 250 V 225 H 300" stroke="var(--color-border-subtle)" strokeWidth="4" fill="none" strokeLinecap="round" strokeLinejoin="round" />
              ...
              <path d="M 100 75 H 150 V 150 H 200" stroke="var(--color-primary-500)" strokeOpacity="0.6" strokeWidth="4" fill="none" strokeLinecap="round" strokeLinejoin="round"
                  className={`transition-opacity duration-700 delay-300 ${isZoomed ? 'opacity-100' : 'opacity-0'}`} />
              <path d="M 200 150 H 250 V 225 H 300" stroke="var(--color-primary-400)" strokeOpacity="0.6" strokeWidth="4" fill="none" strokeLinecap="round" strokeLinejoin="round"
                  className={`transition-opacity duration-700 delay-500 ${isZoomed ? 'opacity-100' : 'opacity-0'}`} />

              <path d="M 100 75 H 150 V 150 H 200" stroke="var(--color-primary-500)" strokeWidth="2.5" fill="none" strokeDasharray="8 8" strokeLinecap="round" strokeLinejoin="round"
                  className={`transition-opacity duration-700 delay-300 ${isZoomed ? 'opacity-100' : 'opacity-0'}`}>
               <animate attributeName="stroke-dashoffset" from="32" to="0" dur="0.8s" repeatCount="indefinite" />
            </path>
              <path d="M 200 150 H 250 V 225 H 300" stroke="var(--color-info-400)" strokeWidth="2.5" fill="none" strokeDasharray="8 8" strokeLinecap="round" strokeLinejoin="round"
                  className={`transition-opacity duration-700 delay-500 ${isZoomed ? 'opacity-100' : 'opacity-0'}`}>
               <animate attributeName="stroke-dashoffset" from="32" to="0" dur="0.8s" repeatCount="indefinite" />
            </path>
         </svg>
         ...
         <div className="flex justify-between items-start mb-4 w-full">
           <div className="bg-[var(--color-info-light)] border border-[var(--color-info-border)] text-[var(--color-info-400)] text-[10px] sm:text-xs font-bold px-2 py-1 rounded-md font-mono shrink-0">BNA401</div>
             <button aria-label="إغلاق لوحة المعلومات" className="text-[var(--color-text-muted)] hover:text-white transition-colors bg-white/5 hover:bg-white/10 p-1.5 rounded-lg border border-white/5"><X size={14}/></button>
         </div>
         ...
         <div className="grid grid-cols-2 gap-2 mb-4">
           <div className="bg-white/5 p-2 rounded-xl text-center border border-white/5">
              <div className="text-[10px] text-[var(--color-text-muted)] mb-1 font-medium">السنة</div>
              <div className="font-bold text-white text-sm">الثالثة</div>
            </div>
            <div className="bg-white/5 p-2 rounded-xl text-center border border-white/5">
              <div className="text-[10px] text-[var(--color-text-muted)] mb-1 font-medium">الساعات</div>
         ...
         <div className="flex flex-col gap-2 mt-auto">
           <div>
             <div className="bg-[var(--color-primary-600)]/10 border border-[var(--color-primary-400)]/10 text-[var(--color-text-secondary)] p-2 sm:p-2.5 rounded-xl flex justify-between items-center shadow-inner">
                  <span className="text-[10px] sm:text-xs text-[var(--color-primary-400)] font-bold flex items-center gap-1.5 truncate"><div className="w-1 h-1 rounded-full bg-[var(--color-primary-400)] shrink-0"></div> المتطلبات السابقة: <span className="text-[var(--color-text-secondary)] font-medium">الجبر الخطي</span></span>
                  <span className="bg-[var(--color-primary-400)]/20 px-1.5 py-0.5 rounded-md text-[9px] text-[var(--color-primary-400)] font-mono font-bold shrink-0">BLA401</span>
              </div>
           </div>

           <div>
              <div className="bg-[var(--color-info-400)]/10 border border-[var(--color-info-border)] text-[var(--color-text-secondary)] p-2 sm:p-2.5 rounded-xl flex justify-between items-center shadow-inner">
                 <span className="text-[10px] sm:text-xs text-[var(--color-info-400)] font-bold flex items-center gap-1.5 truncate"><div className="w-1 h-1 rounded-full bg-[var(--color-info-400)] shrink-0"></div> يفتح المقررات: <span className="text-[var(--color-text-secondary)] font-medium">البيانات</span></span>
        ...
        >
           <MousePointer2 className={`w-8 h-8 ${step >= 3 ? 'text-[var(--color-info-300)] fill-[var(--color-info-400)]/20' : 'text-[var(--color-text-secondary)] fill-white/10'} transition-colors duration-300 transform -scale-x-100`} />
        </div>
    </div>
  );
};
