...
const MapNode = ({ title, code, top, left, isActive, isCompleted }: MapNodeProps) => (
  <div 
    className={`absolute w-36 md:w-44 -translate-x-1/2 -translate-y-1/2 rounded-2xl p-4 border transition-all duration-200 flex flex-col items-center justify-center 
      ${isActive ? 'border-indigo-400 bg-[var(--color-bg-secondary)] shadow-lg scale-[1.15] z-20 ring-1 ring-indigo-400/50' : 
        isCompleted ? 'border-indigo-500/50 bg-[var(--color-bg-secondary)] z-10 shadow-md' : 
        'border-slate-700/60 bg-[var(--color-bg-overlay)] opacity-70 z-10 grayscale-[0.5]'}`}
    style={{ top, left }}
  >
    <div className={`text-[10px] md:text-xs px-2.5 py-1 rounded-md flex items-center gap-1.5 mb-2.5 font-mono font-bold tracking-wide ${isActive ? 'bg-indigo-500/10 text-indigo-400' : 'bg-[var(--color-bg-elevated)] text-slate-400'}`}>
    ...
    <div className="w-full h-full bg-transparent overflow-hidden relative select-none">
        ...
        <div className={`absolute inset-0 z-10 transition-transform duration-[1200ms] ease-[cubic-bezier(0.22,1,0.36,1)] origin-center ${isZoomed ? 'scale-[1.15] md:-translate-x-[10%] -translate-y-[10%]' : 'scale-100 translate-x-0 translate-y-0'}`}>
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none drop-shadow-xl" viewBox="0 0 400 300" preserveAspectRatio="none">
            ...
            <path d="M 100 75 H 150 V 150 H 200" stroke="var(--color-indigo-600)" strokeOpacity="0.6" strokeWidth="4" fill="none" strokeLinecap="round" strokeLinejoin="round"
                  className={`transition-opacity duration-700 delay-300 ${isZoomed ? 'opacity-100' : 'opacity-0'}`} />
            <path d="M 200 150 H 250 V 225 H 300" stroke="var(--color-indigo-400)" strokeOpacity="0.6" strokeWidth="4" fill="none" strokeLinecap="round" strokeLinejoin="round"
                  className={`transition-opacity duration-700 delay-500 ${isZoomed ? 'opacity-100' : 'opacity-0'}`} />

            <path d="M 100 75 H 150 V 150 H 200" stroke="var(--color-indigo-600)" strokeWidth="2.5" fill="none" strokeDasharray="8 8" strokeLinecap="round" strokeLinejoin="round"
                  className={`transition-opacity duration-700 delay-300 ${isZoomed ? 'opacity-100' : 'opacity-0'}`}>
               <animate attributeName="stroke-dashoffset" from="32" to="0" dur="0.8s" repeatCount="indefinite" />
            </path>
            <path d="M 200 150 H 250 V 225 H 300" stroke="var(--color-indigo-400)" strokeWidth="2.5" fill="none" strokeDasharray="8 8" strokeLinecap="round" strokeLinejoin="round"
                  className={`transition-opacity duration-700 delay-500 ${isZoomed ? 'opacity-100' : 'opacity-0'}`}>
               <animate attributeName="stroke-dashoffset" from="32" to="0" dur="0.8s" repeatCount="indefinite" />
            </path>
         </svg>
         ...
         <MapNode title="البيانات" code="BCG601" top="75%" left="75%" isCompleted={false} />
          ...
          <div 
            className={`absolute bottom-4 left-4 right-4 sm:left-auto sm:right-6 sm:w-72 bg-[var(--color-bg-primary)] border border-white/8 shadow-[var(--shadow-card)] p-4 sm:p-5 rounded-3xl transition-all duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] z-30 flex flex-col ${isZoomed ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-8 scale-95 pointer-events-none'}`}
            dir="rtl"
          >
          <div className="flex justify-between items-start mb-4 w-full">
            <div className="bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-[10px] sm:text-xs font-bold px-2 py-1 rounded-md font-mono shrink-0">BNA401</div>
         ...
         <div className="grid grid-cols-2 gap-2 mb-4">
          ...
          <div className="flex flex-col gap-2 mt-auto">
             <div>
               <div className="bg-indigo-600/10 border border-indigo-500/10 text-slate-200 p-2 sm:p-2.5 rounded-xl flex justify-between items-center shadow-inner">
                  <span className="text-[10px] sm:text-xs text-indigo-400 font-bold flex items-center gap-1.5 truncate"><div className="w-1 h-1 rounded-full bg-indigo-400 shrink-0"></div> المتطلبات السابقة: <span className="text-slate-300 font-medium">الجبر الخطي</span></span>
                  <span className="bg-indigo-500/20 px-1.5 py-0.5 rounded-md text-[9px] text-indigo-400 font-mono font-bold shrink-0">BLA401</span>
               </div>
             </div>

             <div>
               <div className="bg-indigo-500/10 border border-indigo-500/20 text-slate-200 p-2 sm:p-2.5 rounded-xl flex justify-between items-center shadow-inner">
                  <span className="text-[10px] sm:text-xs text-indigo-400 font-bold flex items-center gap-1.5 truncate"><div className="w-1 h-1 rounded-full bg-indigo-400 shrink-0"></div> يفتح المقررات: <span className="text-slate-300 font-medium">البيانات</span></span>
                  <span className="bg-indigo-500/10 px-1.5 py-0.5 rounded-md text-[9px] text-indigo-400 font-mono font-bold shrink-0">BCG601</span>
               </div>
             </div>
          </div>
        </div>
        ...
        >
           <MousePointer2 className={`w-8 h-8 ${step >= 3 ? 'text-indigo-300 fill-indigo-400/20 drop-shadow-md' : 'text-slate-200 fill-white/10 drop-shadow-lg'} transition-colors duration-300 transform -scale-x-100`} />
        </div>
    </div>
  );
};
