...
export class ErrorBoundary extends Component<Props, BoundaryState> {
  ...
  public render() {
    if (this.state.hasError) {
      return (
        <div className="flex flex-col items-center justify-center p-8 bg-[var(--color-bg-primary)]/80 rounded-2xl border border-[var(--color-danger-border)] text-center m-4">
          <h2 className="text-2xl font-bold text-[var(--color-danger-400)] mb-4">عذراً، حدث خطأ غير متوقع.</h2>
          <p className="text-[var(--color-text-muted)]">نحن نعمل على حل هذه المشكلة.</p>
          <button
            type="button"
