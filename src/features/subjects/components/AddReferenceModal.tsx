...
import type { ReferenceType, SubjectReferenceInsert, SubjectReference } from '../src/types';
...
type AddReferenceModalProps = {
  ...
  onClose: () => void;
  courseCode: string;
  onAdd: (reference: SubjectReferenceInsert) => Promise<{ data: SubjectReference | null; error: Error | null }> | undefined;
  isAdding: boolean;
};
...
export function AddReferenceModal({ isOpen, onClose, courseCode, onAdd, isAdding }: AddReferenceModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await onAdd({ course_code: courseCode, type, title: title.trim(), url: url.trim(), description: description.trim() || undefined });
    setType('reference');
    setTitle('');
    setUrl('');
    setDescription('');
      ...
      <GlassCard className="relative w-full max-w-lg">
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-white">إضافة مصدر</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              <Button type="submit" variant="primary" isLoading={isAdding} className="flex-1">
