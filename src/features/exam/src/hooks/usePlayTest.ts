...
import { getTestById, rateTest } from '../lib/store';
...
export function usePlayTest(testId: string | undefined, navigate: (path: string) => void): UsePlayTestReturn {
  ...
  const onKeyDownRaw = useCallback((e: globalThis.KeyboardEvent) => {
    ...
    if (!Number.isNaN(num) && num >= 1 && num <= 9) {
      ...
      const opts = currentQ.type === 'true_false'
        ? ['true', 'false']
        : currentQ.options;
      if (!opts) return;
      const idx = num - 1;
  ...
  const handleRateTest = useCallback((stars: number) => {
    ...
    if (result.success) {
      setTest(prev => prev ? { ...prev, rating: stars, ratedBySession: true } : prev);
