...
import { Users, BookOpen, Calendar, GraduationCap, MessageCircle, Link2, Plus, Loader2, Pencil } from 'lucide-react';
...
export function GroupFormModal({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const initialMajor = mode === 'edit' && group ? group.major : userMajor;
      if (initialMajor) {
        setSelectedMajor(initialMajor);
        getCoursesByMajor(initialMajor).then(setCourses);
      } else {
        setCourses([]);
      }
    }
  }, [isOpen, mode, group, userMajor, getCoursesByMajor]);
  ...
  const validate = (): boolean => {
    ...
    if (mode === 'create') {
      ...
      newErrors.course = 'يرجى اختيار المادة';
    }
    ...
    <ModalShell isOpen={isOpen} onClose={onClose} maxWidth="max-w-lg" closeButton>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <Button
          ...
          variant="primary"
          icon={isSubmitting ? <Loader2 className="w-5 h-5 animate-spin" /> : (isEdit ? <Pencil className="w-5 h-5" /> : <Plus className="w-5 h-5" />)}
        >
          {isSubmitting ? (isEdit ? 'جاري التحديث...' : 'جاري الإنشاء...') : (isEdit ? 'تحديث المجموعة' : 'إنشاء المجموعة')}
        </Button>
      </form>
    </ModalShell>
  );
