...
export const refreshProfile = async (userId: string): Promise<RefreshProfileResult> => {
  ...
  try {
    ...
    const { data, error } = await client
      .from('profiles')
      .select('id, full_name, avatar_url, phone, major, level, current_semester, role, created_at, updated_at')
      .eq('id', userId)
      .maybeSingle();
...
export const updateProfile = async (userId: string, full_name: string, username: string, major?: string, level?: string, current_semester?: string): Promise<UpdateProfileResult> => {
  if (!hasSupabaseEnv()) {
    return { data: null, error: createMissingEnvError() };
  }
