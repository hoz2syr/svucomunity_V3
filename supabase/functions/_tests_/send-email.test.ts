...
import { verifyCaller } from '../send-email/index.ts';
import { createMockSupabase } from './mocks/supabase.ts';

describe('send-email/verifyCaller', () => {
  let supabase;
  ...
  it('returns 403 when user is not admin or not active', async () => {
    supabase.auth.getUser.mockResolvedValueOnce({
      data: { user: { id: 'user-1' } },
      error: null,
    });
    supabase.maybeSingle.mockResolvedValueOnce({
      data: { is_admin: true, is_active: false },
      error: null,
    });
    const result = await verifyCaller(supabase, 'Bearer valid-token');
    expect(result.ok).toBe(false);
  ...
  it('returns profile for valid active admin user', async () => {
    supabase.auth.getUser.mockResolvedValueOnce({
      data: { user: { id: 'user-1' } },
      error: null,
    });
    const mockProfile = { is_admin: true, is_active: true };
