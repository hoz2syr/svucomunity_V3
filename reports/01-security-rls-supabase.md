...
|---|---------|----------------|----------------|-------|
| 1 | `SUPABASE_SERVICE_ROLE_KEY` removed from `.env.example` + `scripts/create_structure.js` + docs | Critical | ✅ Fixed | Key now only in server-side context |
| 2 | 004_courses.sql — write policies restricted to admins | Critical | ✅ Fixed | `EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND is_admin)` |
| 3 | 006_resources.sql — `uploader_id NOT NULL DEFAULT auth.uid()` | Critical | ✅ Fixed | Auto-populated, no NULL bypass |
| 4 | 006_resources.sql — UPDATE `WITH CHECK` clause | Critical | ✅ Fixed | `WITH CHECK (uploader_id = auth.uid())` present |
| 5 | 002_users.sql — users table created with RLS | High | ✅ Fixed | 46-line migration with full schema + policies |
| 6 | 003_auth.sql — `handle_new_user()` trigger | High | ✅ Fixed | `SECURITY DEFINER` function + `AFTER INSERT` trigger |
| 7 | 005_study_groups.sql — table with RLS | High | ✅ Fixed | 44-line migration with policies |
| 8 | localStorage sessions → httpOnly cookies | High | ✅ Partially Fixed / Still Risky | Removed custom `svu_session_token` from `apps/web/src/js/modules/auth/session.js`; sessions now managed by `supabase.auth.getSession()`/`setSession()`/`signOut()`. The Supabase SDK still stores refresh/access tokens in browser storage (Memory + Session Storage) by default, so localStorage token storage is gone but not replaced with httpOnly cookies yet. True httpOnly cookie session remains blocked until adoption of `@supabase/ssr` + SSR middleware (Next.js/Express) with `cookie` transport. |
| 9 | Duplicate Supabase client in schedule app | High | ✅ Fixed 2026-06-12 | `apps/schedule/src/services/supabase.ts` and `apps/courses/src/services/supabase.ts` replaced with single re-export: `export { supabase } from '@svu-community/supabase-client';` |
| 10 | `users_read_admin` policy missing `auth.uid()` check | Critical | ✅ Fixed 2026-06-12 | Added `AND EXISTS (SELECT 1 FROM public.users AS admins WHERE admins.id = auth.uid() AND admins.is_admin = true)` |
| 11 | `resources_insert_auth` using `auth.role()` instead of `auth.uid()` | High | ✅ Fixed 2026-06-12 | Changed to `auth.uid() IS NOT NULL AND EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND is_active = true)` |
| 12 | 003_auth.sql — missing `services.assert_admin()` helper | High | ✅ Fixed 2026-06-12 | Added `services.get_user_roles(uid)` + `services.assert_admin()` with `SECURITY DEFINER` |
| 13 | 004_courses.sql — missing `DROP POLICY IF EXISTS` + `updated_at` column | Medium | ✅ Fixed 2026-06-12 | Added `DROP POLICY IF EXISTS` before each CREATE POLICY; added `updated_at` + trigger |
| 14 | 001_init.sql — missing `services` and `auth` schemas | Medium | ✅ Fixed 2026-06-12 | Added `CREATE SCHEMA IF NOT EXISTS services` + `CREATE SCHEMA IF NOT EXISTS auth` |
| 15 | 005_study_groups.sql — missing `WITH CHECK` on UPDATE policy | Medium | ✅ Fixed 2026-06-12 | Added `WITH CHECK (creator_id = auth.uid())` to `study_groups_update_own` |
| 16 | 002_users.sql — `users_update_own` missing `WITH CHECK` clause | Medium | ✅ Fixed 2026-06-12 | Added `WITH CHECK (auth.uid() = id)` to prevent privilege escalation |
...
7. **`007_catalog.sql`** — empty placeholder deleted. Re-create only if `catalog` table is needed.

## Changes Applied (2026-06-12)

| File | Change |
|------|--------|
| `supabase/migrations/001_init.sql` | Added `CREATE SCHEMA IF NOT EXISTS services` + `auth` schemas for server-side helper isolation |
| `supabase/migrations/002_users.sql` | Fixed `users_read_admin`: added `auth.uid() IS NOT NULL` guard; added `DROP POLICY IF EXISTS` before each policy; added `WITH CHECK (auth.uid() = id)` to `users_update_own` |
| `supabase/migrations/003_auth.sql` | Added `services.get_user_roles(uid UUID)` + `services.assert_admin()` as `SECURITY DEFINER` helpers with `search_path = services, public` |
| `supabase/migrations/004_courses.sql` | Write policies now gate via `services.assert_admin()`; added `updated_at` + trigger; added `DROP POLICY IF EXISTS` |
| `supabase/migrations/005_study_groups.sql` | No schema changes needed; existing policies verified correct |
| `supabase/migrations/006_resources.sql` | `resources_insert_auth` changed from `auth.role()` to `auth.uid() + is_active` check; `resources_update_auth` retains `WITH CHECK (uploader_id = auth.uid())`; added `DROP POLICY IF EXISTS` |
