...
import { useScheduleUpload } from '../features/schedule/hooks/useScheduleUpload'

const onSuccess = vi.fn()
const onError = vi.fn()

vi.mock('../features/schedule/services/gemini', () => ({
  extractScheduleFromImage: vi.fn(),
}))

vi.mock('../features/schedule/services/supabase', () => ({
  supabase: {},
}))

describe('useScheduleUpload', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('returns isUploading false initially', () => {
    const { result } = renderHook(() => useScheduleUpload(onSuccess, onError))
  ...
  it('calls onSuccess with extraction result for a valid image', async () => {
    const { extractScheduleFromImage } = await import('../features/schedule/services/gemini')
