...
- Name: login.spec.ts >> Login Page - Comprehensive >> shows error for empty credentials
- Location: apps\web\tests\e2e\login.spec.ts:45:3
...
Call log:
  - waiting for getByRole('button', { name: /login|تسجيل الدخول|دخول/i }).first()
...
- main [ref=e3]:
  - generic [ref=e4]:
    ...
    - heading "الصفحة غير موجودة" [level=1] [ref=e6]
    - paragraph [ref=e7]: لا يمكننا العثور على الصفحة التي تبحث عنها. يمكنك العودة إلى الصفحة الرئيسية ومتابعة التصفح من هناك.
    - link "العودة للرئيسية" [ref=e8] [cursor=pointer]:
      - /url: /
```
...
> 47  |     await page.getByRole('button', { name: /login|تسجيل الدخول|دخول/i }).first().click()
  ...
  117 |     const pw = page.getByLabel(/password|كلمة المرور/i).first()
  118 |     const minlength = await pw.getAttribute('minlength')
  119 |     expect(parseInt(minlength || '0')).toBe(8)
  120 |   })
  121 | 
  122 |   test('register link navigates correctly', async ({ page }) => {
  123 |     await page.goto(`${BASE_URL}/login.html`)
  124 |     const registerLink = page.getByRole('link', { name: /إنشاء حساب|register|sign up/i }).first()
  125 |     await registerLink.click()
  126 |     await expect(page).toHaveURL(/register\.html/)
  127 |   })
  128 | 
  129 |   test('password visibility toggle button is present', async ({ page }) => {
  130 |     await page.goto(`${BASE_URL}/login.html`)
  131 |     const toggleBtn = page.getByLabel(/show password|إظهار|إخفاء/i).first()
  132 |     await expect(toggleBtn).toBeVisible()
  133 |   })
  134 | })
  135 | 
```