...
- Name: login.spec.ts >> Login Page - Comprehensive >> login page has link to register
- Location: apps\web\tests\e2e\login.spec.ts:32:3
...
Locator: getByRole('link', { name: /إنشاء حساب|register|sign up|تسجيل/i }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('link', { name: /إنشاء حساب|register|sign up|تسجيل/i }).first()
